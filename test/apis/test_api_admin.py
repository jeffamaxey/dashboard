...
class ApiAccessControlTest(BaseTestCase):
    ...
    def test_authentication_failure(self):
        response = self.client.get(f'/api/projects/{TEST_PROJECT_ID}/applications')
        self.assertEqual(401, response.status_code)

    def test_authentication_success(self):
        token = self._get_token(TEST_AUTH_ID_1)
        response = self.client.get(
            f'/api/projects/{TEST_PROJECT_ID}/applications',
    ...
    def test_get_users(self):
        ...
        response = self.client.get(
            f'/api/projects/{TEST_PROJECT_ID}/applications/{TEST_APPLICATION_ID}/acl',
            headers=headers,
        )
        self.assertEqual(200, response.status_code)
